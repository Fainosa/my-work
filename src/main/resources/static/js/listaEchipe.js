...
$(document).ready(function () {
    ...
    $('#echipaTable ').on('click', 'tr', function () {
        ...
        var getJucatorByIdUrl = "http://localhost:8088/jucator/echipa/" + data.id;
        getJucatorById(getJucatorByIdUrl);

        var getAntrenorByIdUrl = "http://localhost:8088/antrenor/echipa/" + data.id;
        getAntrenorById(getAntrenorByIdUrl);

        listaJucatori.innerHTML += "<li style='text-align: center; font-size: 20px; font-family: serif;'>JUCATORI</li>";

        $.each(jucatori, function(i, item) {
            listaJucatori.innerHTML += "<li style='text-align: center; font-size: 20px; font-family: serif;'>" +
                jucatori[i].nume + "  " + jucatori[i].prenume + "</li>";
        });

        listaJucatori.innerHTML += "<br/><li style='text-align: center; font-size: 20px; font-family: serif;'>ANTRENOR</li>";
        console.log("ascascascas",antrenori)

        $.each(antrenori, function(i, item) {
            listaJucatori.innerHTML += "<li style='text-align: center; font-size: 20px; font-family: serif;'>" +
                antrenori[i].nume + "  " + antrenori[i].prenume + "</li>";
        });
